...
namespace tally_device {
  ...
  SpatialField::SpatialField(TallyGlobalState *s)
  ...
  StructuredRegularField::StructuredRegularField(TallyGlobalState *s)
    : SpatialField(s)
  {}
  ...
  TallySpatialField::SP StructuredRegularField::createTallySpatialField(
                                                                        TallyModel::SP model, int slot) const
  {
    if (!isValid())
      return {};
    auto ctx = deviceState()->context;
    BNScalarType tallyType;
    switch (m_data->elementType()) {
    case ANARI_FLOAT32:
      tallyType = BN_SCALAR_FLOAT;
      break;
    case ANARI_UINT8:
      tallyType = BN_SCALAR_UINT8;
      break;
      // case ANARI_FLOAT64:
    ...
    auto field
      = TallyStructuredData::create(this);
                                    // (const int3 &)dims,
                                    // tallyType,
                                    // m_data->data(),
                                    // (const float3 &)m_origin,
                                    // (const float3 &)m_spacing);
      // = bnStructuredDataCreate(model,
      //                                   slot,
      //                                   (const int3 &)dims,
      //                                   tallyType,
      //                                   m_data->data(),
  ...
  UnstructuredField::UnstructuredField(TallyGlobalState *s) : SpatialField(s) {}
  ...
  TallySpatialField::SP UnstructuredField::createTallySpatialField(
                                                                   TallyModel::SP model, int slot) const
  {
    auto ctx = deviceState()->context;
    std::cout << "==================================================================" << std::endl;
    ...
    std::cout << "==================================================================" << std::endl;
    return TallyUMeshField::create(this);
    // bnUMeshCreate(model,
    //                      slot,
  ...
  BlockStructuredField::BlockStructuredField(TallyGlobalState *s)
    : SpatialField(s)
  {}
  ...
  TallySpatialField::SP BlockStructuredField::createTallySpatialField(
                                                                      TallyModel::SP model, int slot) const
  {
#if TALLY
...
TALLY_ANARI_TYPEFOR_DEFINITION(tally_device::SpatialField *);
